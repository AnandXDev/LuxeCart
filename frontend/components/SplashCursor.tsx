...
const SplashCursor: React.FC<SplashCursorProps> = ({
  ...
  useEffect(() => {
    ...
    class PointerInstance implements Pointer {
      id = -1;
      texcoordX = 0;
      texcoordY = 0;
      prevTexcoordX = 0;
    ...
    const getWebGLContext = (canvas: HTMLCanvasElement) => {
      ...
      if (isWebGL2) {
        ...
        formatRG = getSupportedFormat(gl, gl.RG16F, (gl as any).RG, halfFloatTexType);
        formatR = getSupportedFormat(gl, gl.R16F, (gl as any).RED, halfFloatTexType);
      } else {
    ...
    const supportRenderTextureFormat = (gl: WebGLRenderingContext, internalFormat: number, format: number, type: number) => {
    ...
    const getSupportedFormat = (gl: WebGLRenderingContext, internalFormat: number, format: number, type: number): { internalFormat: number, format: number } | null => {
